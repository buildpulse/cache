...
export async function saveImpl(
    ...
    try {
        ...
        for (const cachePath of cachePaths) {
            const repoId = process.env.GITHUB_REPOSITORY_ID || 'unknown';
            const s3Key = `${repoId}/${primaryKey}/${path.basename(cachePath)}`;
            try {
        ...
        if (error instanceof Error) {
            utils.logWarning(`Error saving cache to S3: ${error.message}`);
        } else {
